...
export class RoleManageComponent{
  ...
  constructor( private scepterService:ScepterServiceNs.ScepterService,private showMessageService:ShowMessageService,
  ...
  public deleteSingle(index:number,role:ScepterServiceNs.RoleModel){
    let modalCtrl = this.showMessageService.showAlertMessage("","您确定要删除吗？","confirm");
  ...
  public deleteSelected(){
    if(this.selectedRolesId.length === 0){
      this.showMessageService.showAlertMessage("","请选择要删除的角色","info");
      return;
    }
  ...
  public cancelTabPage(){
    ...
    if(this.tabPage === "privilege"){
      ...
      for(let key in this.addRoleForm.controls){
    ...
    Observable.forkJoin([this.scepterService.getMenuShown(),this.scepterService.getMenusAuths(roleId)])
      .subscribe((resData:ScepterServiceNs.ScepterResModelT<any>[]) => {
        if(resData[0].code !== 0 || resData[1].code !== 0){
          let message = resData[0].code !== 0 ? resData[0].message : resData[1].message;
