...
import { DeptServiceNs } from "../../../core/common-services/dept.service";
...
export class DeptManageComponent implements OnInit {
  ...
  constructor(private deptService:DeptServiceNs.DeptService,private messageService:ShowMessageService) {
    this.tabPageIndex = 0;
  ...
  public addDept(name:string,parent?:DeptServiceNs.DeptItemModel){
    ...
    this.deptService.insertDept(parentId,name).subscribe((resData:any) => {
      ...
      let temp:DeptServiceNs.DeptItemModel = {
        id:resData.value,
        name:name,
        children:[],
        leaf:1,
  ...
  public deleteDept(item:DeptServiceNs.DeptItemModel,parent?:DeptServiceNs.DeptItemModel){
    this.messageService.showAlertMessage("",`您确定要删除${item.name}`,"success").afterClose
      .subscribe((type:string)=>{
        ...
        this.deptService.removeDept(item.id).subscribe((resData:any) => {
          ...
          this.messageService.showToastMessage("操作成功","success");
          let list = parent ? parent.children:this.deptDataList;
