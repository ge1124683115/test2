...
export class UserManageComponent implements OnInit {
  ...
  constructor( private userService:UserServiceNs.UserService,private messageService:ShowMessageService,
                private formBuilder:FormBuilder,private validator:UfastValidatorsService,
               private scepterService:ScepterServiceNs.ScepterService
  ) {
  ...
  public lockUser(lock:number,item:any){
    ...
    let message:string = lock===0?"启用":"锁定";
  ...
  private getDepartmentList(id:string,targetList?:any[]){
    this.userService.getDepartment(id).subscribe((resData:UserServiceNs.UfastHttpAnyResModel) => {
      ...
      let modelList:UserServiceNs.DepartmentModel[] = <UserServiceNs.DepartmentModel[]>resData.value;
  ...
  public addOrEditSubmit(){
    ...
    for(let key in this.userInfoForm.controls){
    ...
    let data:UserServiceNs.UserInfoModel = {
      loginName:this.userInfoForm.value.loginName,
      name:this.userInfoForm.value.name,
      nickname:this.userInfoForm.value.nickname,
      sex:parseInt(this.userInfoForm.value.sex),
      locked:parseInt(this.userInfoForm.value.locked),
